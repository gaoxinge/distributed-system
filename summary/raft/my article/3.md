## log replication/nonblocking raft/sync transportation
...
```
function HandleMessage(r: Raft, m: Message) Message {
    r.serverChannel <- m
    return <- m.channel
}
```
...
function handleAppendResponse(r: Raft, m: Message) {
    ...
    if r.term == m.term
        ...
        if not m.ok and not m.consistency
            if r.clientIndexes[c.id] == m.prevLogIndex + 1
                r.clientIndexes[c.id]--
        else if m.ok
