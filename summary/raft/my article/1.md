## leader election: sync algorithm
...
```
function handleClient(r: Raft, m: Message) Message {
    r.index++
...
```
function handleVoteRequest(r: Raft, m: Message) Message {
    if r.term > m.term
        ok = false
        goto end
    ...
    if r.term < m.term
        if log-up-to-date
            ...
            r.becomeFollowerWithWrite(r.term, null)
            ok = false
            goto end
...
```
function handleAppendRequest(r: Raft, m: Message) Message {
    if r.term > m.term
        ok = false
        goto end
